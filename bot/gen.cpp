...
{
    uint8_t new_rotation = (static_cast<uint8_t>(this->rotation) + 1) & 0b11;

    if (!field.is_colliding(height, this->x, this->y, Rotation(new_rotation))) {
        this->rotation = Rotation(new_rotation);
        return true;
    }

    if (!field.is_colliding(height, this->x - ROTATION_OFFSET[new_rotation][0], this->y - ROTATION_OFFSET[new_rotation][1], Rotation(new_rotation))) {
...
{
    uint8_t new_rotation = (static_cast<uint8_t>(this->rotation) + 3) & 0b11;
...
bool Position::move_180(Field& field, int height[6])
{
    ...
    uint8_t new_rotation = (static_cast<uint8_t>(this->rotation) + 2) & 0b11;
...
{
    assert(x >= 0 && x <= 5);
    return this->full[x][static_cast<uint8_t>(rotation)];
};

void PlacementMap::set_full(int x, Rotation rotation, bool value)
{
    assert(x >= 0 && x <= 5);
    this->full[x][static_cast<uint8_t>(rotation)] = value;
};
...
void Generator::generate(Field& field, Pair pair, avec<Placement, 22>& placement)
{
    ...
    if (fast_mode) {
        for (uint8_t x = 0; x < 6; ++x) {
            placement.add({ x, Rotation::UP });
        }

        for (uint8_t x = 0; x < 5; ++x) {
            placement.add({ x, Rotation::RIGHT });
        }
        ...
        for (uint8_t x = 0; x < 6; ++x) {
            placement.add({ x, Rotation::DOWN });
        }

        for (uint8_t x = 1; x < 6; ++x) {
            placement.add({ x, Rotation::LEFT });
        }
    }
    else {
...
void Generator::expand(Field& field, int height[6], Position& position, avec<Position, 120>& queue, PositionMap& queue_map)
{
    ...
    if (cw.move_cw(field, height) && !queue_map.get(cw.x, cw.y, cw.rotation) && cw.y < 13 && cw.y + ROTATION_OFFSET[static_cast<uint8_t>(cw.rotation)][1] < 13) {
        queue.add(cw);
        queue_map.set(cw.x, cw.y, cw.rotation, true);
    }

    Position ccw = position;
    if (ccw.move_ccw(field, height) && !queue_map.get(ccw.x, ccw.y, ccw.rotation) && ccw.y < 13 && ccw.y + ROTATION_OFFSET[static_cast<uint8_t>(ccw.rotation)][1] < 13) {
        queue.add(ccw);
