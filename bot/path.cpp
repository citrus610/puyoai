...
void PathFinder::expand(Field& field, int height[6], PathFinderNode& node, std::vector<PathFinderNode>& queue, PathPositionMap& queue_map)
{
    ...
    if (cw.position.move_cw(field, height) && cw.position.y < 13 && cw.position.y + ROTATION_OFFSET[cw.position.rotation][1] < 13) {
    ...
    if (ccw.position.move_ccw(field, height) && ccw.position.y < 13 && ccw.position.y + ROTATION_OFFSET[ccw.position.rotation][1] < 13) {
