...
public:
    ...
    void clear();
public:
    uint32_t hash(Field& field);
public:
    bool get_entry(uint32_t hash, uint16_t& depth);
    bool add_entry(uint32_t hash, uint16_t depth);
public:
    double hashful();
};

static bool operator < (const TEntry& a, const TEntry& b)
{
    if (a.hash == 0 && a.depth == 0) {
        return true;
    }
